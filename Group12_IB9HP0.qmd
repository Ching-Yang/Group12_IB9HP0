...
This report simulates a real-world e-commerce data environment engaging in end-to-end data management. The report is composed of 4 major tasks from database design to data analysis.
...
![E-R Diagram](Report_images/E-R_Diagram.png){fig-align="center"}

Our E-R Diagram that explains the e-commerce platform is composed of entities and relationships including Customers, Orders, Products, Suppliers, Order items, Advertisements, Payments, and Shipments (Figure 1) which can be explained in the relationship sets below.

**Customers:** The diagram begins with the Customer entity which has a unique customer ID as its primary key. The information stored as attributes in this entity would relate to the customer’s personal details, address, and account information (i.e., gender, postcode, account type). Additionally, a (1:N) self-referencing relationship is seen between the customer entity and the referral. This relationship suggests a customer can be a referrer and referee (Figure 2).

![Customer Referral relationship](Report_images/refer.jpeg){fig-align="center" width="302"}

**Orders:** The Customer entity shares a many to many (M:N) relationship with Products entity through the relationship Orders. As such, Orders contains the primary key "order_id" with the attributes order status and order date.

![Customer and Orders relationship](Report_images/cust_order.jpeg){fig-align="center" width="339"}

**Suppliers:** Supplier sells Products on the e-commerce platform with a one to many (1:N) relationship (Figure 4) as it is assumed that each supplier may sell multiple different products but the products are unique across suppliers. Attributes recorded pertain to the account information, contact, and name of the supplier.

![Suppliers and Products relationship](Report_images/supp_pd.jpeg){fig-align="center" width="350"}

**Products:** The product entity contains attributes that describe the product (product names, dimensions, description etc.) as well as categories.

**Order items:** After a customer places an order, the product information is contained in Order Items entity. The Order Items and Product entities share a one to many (1:N) relationship (Figure 5) as it assumes that one Order Item with a unique order item ID (primary key), contains all the products a customer wishes to buy. The entity takes note of the number of products in it and the total price.

![Order items and Products relationship](Report_images/order_item_pd.jpeg){fig-align="center" width="365"}

**Payments:** An (N:1) relationship is created between Order Items and Payments (Figure 6). This suggests that many order Items can be paid in one payment. The primary key for Payment is the payment_id, followed by the attributes of Payment_method and Payment_status.

![Order items and Payments relationship](Report_images/order_pay.jpeg){fig-align="center" width="343"}

**Shipments:** Another (N:1) relationship is witnessed in Order items and Shipments (Figure 7). The relationship implies that many order items can be shipped in one shipment. This entity consists of various attributes such as shipment_date, shipment_status, shipment_company, shipment_postcode, shipment_address, shipment_city and its primary key as the shipment_id.

![Order items and Shipments relationship](Report_images/order_ship.jpeg){fig-align="center" width="344"}

**Advertisements:** Suppliers are able to generate multiple advertisements for their products through the e-commerce platform, hence a one to many (1:N) relationship is shared between them. Ads_id has been assigned as the primary key with attributes ads_name, ads_details, product_discount and discount_price. (Figure 8)

![Suppliers and Advertisements relationship](Report_images/Supp_ads.jpeg){fig-align="center" width="316"}
...
3.  Orders ([order_id]{.underline}, cust_id, order_status, order_date)
...
After that, we created 8 tables that consist of attributes and their data types in below sections.

```{r defineconnection}
#Define connection to connect R with the database using package RSQLite.
#We defined our e-commerce database as "ecomm.db"
connection <- RSQLite::dbConnect(RSQLite::SQLite(),"ecomm.db")
```

After that, we created tables for each entity and relationship in below sections:
...
CREATE TABLE IF NOT EXISTS Orders (
  ...
  order_status VARCHAR (50) NOT NULL CHECK (order_status IN ('Pending', 
...
CREATE TABLE IF NOT EXISTS Suppliers (
  ...
  supplier_email VARCHAR(100) NOT NULL UNIQUE CHECK (supplier_email 
  LIKE '%@%.%'),
  supplier_contact VARCHAR(11) NOT NULL CHECK (supplier_contact 
  NOT LIKE '%[^0-9]%')
);
...
CREATE TABLE IF NOT EXISTS Advertisements (
  ...
  product_discount DECIMAL(3,2) CHECK (product_discount >= 0 AND 
...
CREATE TABLE IF NOT EXISTS Payments (
  ...
  payment_status VARCHAR(50) NOT NULL CHECK (payment_status IN ('Pending', 
  'Processing', 'Completed', 'Failed', 'Cancelled', 'Refund Completed', 
  'Refund Processing', 'Refund Failed')),
  payment_method VARCHAR(50) NOT NULL CHECK (payment_method IN ('Credit Card', 
  'Debit Card', 'PayPal', 'Bank Transfer')),
  FOREIGN KEY('cust_id')
    REFERENCES Customers('cust_id'),
...
CREATE TABLE IF NOT EXISTS Shipments (
  ...
  shipment_status VARCHAR (20) CHECK (shipment_status IN ('Pending', 
...
There are 2 main packages that we utilised in Python, "Faker" to generate realistic names and addresses for individuals in a data set, and "NumPy" to generate random numerical data such as income levels or ages to accompany this demographic information. This combination allows us to create diverse synthetic data sets for testing algorithms, building models, or simulating scenarios without relying on real-world data.

To ensure that there are relationships among our entities and to be able to join the table for future uses, we sequenced the order of creating each table and inserted foreign keys for each table as well. For example, in the product table, we set records (num_records = 80) then generated supplier IDs for the products by randomly selecting from supplier IDs in the Suppliers data frame (supplier_ids_from_Suppliers = np.random.choice(Suppliers\['supplier_id'\], num_records, replace=True))

Another important assumption for our group is that, we set the e-commerce to be UK based, which means all customers and suppliers are all located in the UK. Hence, the phone number and postcode logic are UK-based where the phone number has 11 digits and starts with 07x. For the postcode, we retracted the information from a service provider specialising in postcodes called 'IdealPostcodes' (ideal-postcodes.co.uk. (2023). UK Postcode Format. \[online\] Available at: https://ideal-postcodes.co.uk/guides/uk-postcode-format \[Accessed 16 March 2024\]). and doogal.co.uk. (2022). Postcode Districts. \[online\] Available at: https://www.doogal.co.uk/PostcodeDistricts#google_vignette \[Accessed 16 March 2024\].

![UK Postcode generation](Report_images/postcode.jpeg){fig-align="center" width="497"}

Based on Figure 9, we gathered data from 2 websites, the sector section was where doogal.co.uk provided real information of the UK region, and IdealPostcodes randomly generated the unit part which are streets and buildings with the population information. Notably, the population information is the data from the year 2011.

To reflect the real data, we also incorporated weights to the attributes including Customer age, Product price and Order item quantity in normal distribution, postcode (weighted by population) and payment method (higher weights for Credit Cards and Debit Cards).

Lastly, we have also applied the logic for all status attributes including Order status, Payment status, Shipment status to reflect the real data. For instance, if the order status shows any status apart from 'Pending', the shipment status will appear to be 'In Transit', 'Delivered', or 'Cancelled'. This is to ensure that the status will not conflict to each other such as order status 'Cancelled' would not appear 'In transit' for the same order ID.

### 2.2 Data Import and Quality Assurance

In this step, we imported the data set using the concept of database driver between R, SQL and E-T-L.
...
# If the dataset meets the constraints, the code can be executed without errors. 
# Hence, our dataset has passed the quality and integrity checks.
```
...
#The result shows the NOT NULL and type criteria for each table. 
#Additionally, each table shows primary key as single value. 
#Hence, there is no duplication in the records. 
```
...
GitHub workflows streamline the software development lifecycle, making it more efficient, reliable, and collaborative. To collaborate out the e-commerce project as a team, we created the repository: [Group12_IB9HP0_Repository](https://github.com/Ching-Yang/Group12_IB9HP0) (Figure 10)

![Group 12 Repository files](Report_images/Repository.jpeg){fig-align="center" width="609"}
...
For our e-commerce project, we chose "plot.R" to plot the graphs and "up.R" to update the database and by automating them, we set up the workflow which contains steps in the figure below (Figure 11).

![Workflow Stages](Report_images/Workflows.png){fig-align="center" width="297"}

To show that we have set the validation for the new data, Figure 12 illustrates what would happened if we put in data that failed the checks.

![Github Validation Checks](Report_images/Checks.jpeg){fig-align="center" width="459"}

Because our graphs have some interactive features, completed functions can only be applied with html link in README file, therefore, we utilised "webshot" packages to change them to png files and saved them in "figures" folder.

Therefore, by incorporating Github workflows, it allows us to connect the database to help us to check new data entries every 24 hours.

## Part 4: Data Analysis and Reporting with Quarto in R

### 4.1 Advanced Data Analysis in R

To conduct advanced data analysis on our e-commerce database, we created 8 plots to illustrate the insights of R by first, executing SQL queries to retrieve data using RSQLite and DBI functions, then creating the new data frame for the analysis that we would like to present. To present our data, we applied various packages in R which are described in the table below.
...
We have analysed the e-commerce database in 8 areas below.

1.  Customer distribution analysis based on the order amounts

```{r}
#Data preparation by creating the data frame for the three joint tables.
...
```{r}
# After that, we created the new data frame called "cust_analysis" 
# To manipulate/analyse the data without interrupting the data above.

# To begin with, we create the new column "cust_year_of_birth" 
# To extract only year of birth data from customer's date of birth
# Then create another column "cust_age" from cust_yob.
...
cust_group <- cust_analysis %>% 
              group_by(age_range = cut(cust_age, breaks = c(0, 20, 30, 40, 50, 
Inf), labels = c("Under 20", "21-30", "31-40", "41-50", "51+"))) %>%
...
cust_analysis$cust_age_range <- cut(cust_analysis$cust_age, breaks = c(0, 20, 
30, 40, 50, Inf), labels = age_ranges, right = FALSE)

# Plot the data as a bar plot
cust_order_plot_gg <- ggplot(data = cust_analysis, aes(x = cust_age_range, 
  y = order_item_quantity)) +
  geom_bar(stat = "identity", fill = "#1f77b4") +
  labs(x = "Age Range", y = "Total Order Items Quantity", 
...
```{r}
# Majority of the platform's customers are those who age from 21-30 and 31-40. 
# This is useful for the platform to target the customers. 
# Easier to launch the campaign or promotions.
```

2.  Analysis of the platform growth

```{r}
# To analyse the platform growth, we use the data from cus_o_oi data frame
order_analysis <- cus_o_oi %>%
  mutate(
    year = lubridate::year(order_date),  # Extract year
    month = lubridate::month(order_date), # Extract month
    order_value = order_item_quantity * order_item_unit_price # order value
  )

# Convert year and month to a date format
order_analysis$date <- as.Date(paste(order_analysis$year, order_analysis$month, 
                                     "01", sep = "-"), "%Y-%m-%d")
...
```{r}
# Mutate new column to specify which suppliers are "Entity" or "Individual"
...
supplier_plot <- ggplot(supplier_analysis, aes(x = supplier_type, y = count, 
  fill = supplier_type)) +
...
```{r}
# Majority of the sellers in e-commerce database is "Individual". 
# This is a useful information for the platform.
```
...
payment_analysis <- payment_df %>%
  ...
  rename(payment_method = `payment_method`, count = `n`) # Renaming count column
...
pie_chart_payment <- ggplot(payment_analysis, aes(x = "", y = count, 
  fill = payment_method)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  scale_fill_manual(values = colors) +
  labs(title = "Payment Analysis", fill = "Payment Method") +
...
```{r}
# The most popular method that is used is "Credit Card" and "Debit Card". 
# This information is useful for the platform to improve the payment options.
```

5.  Shipment Analysis: Geographical map

```{r}
# Create a data frame for shipment. 
# Information of city, longitude and latitude from maps csv

shipment_query <- "SELECT a.shipment_city AS City, 
b.Latitude AS shipment_latitude, b.Longitude AS shipment_longitude
FROM shipments a
...
![Geographic Shipment Heat Map](Report_images/UK_heatmap.png){width="360"}

```{r}
#Based on the result, the heatmap shows where shipment mostly located. 
#This figure is useful for the e-commerce company for delivery centre locations.
```
...
order_df <- mutate(order_df, Date_Ordered = as.POSIXct(order_df$Date_Ordered, 
  origin = "1970-01-01", tz = "UTC"),
  Month = format(Date_Ordered, "%b"))

# Ordering months
order_df$Month <- factor(order_df$Month, levels = c("Jan", "Feb", "Mar", 
...
```{r}
#The order quantity graph allows us to see the growth of each month. 
#Can be seen that the quantity per month averages around 15 orders. 
#However, it is obvious that order quantities decreases significantly in October. 
#Platform may increase order quantities through advertisements this month.
```
...
ads_query <- "SELECT order_id AS Orders, ads_id AS Ads_ID, 
order_item_quantity AS Quantity_Ordered
FROM order_items;"

ads_order_items_df <- DBI::dbGetQuery(connection, ads_query)

# Creating new column for presence of Advertisement
ads_order_items_df <- mutate(ads_order_items_df, 
Ads_Present = ifelse(is.na(Ads_ID), 
"No Advertisement", "Advertisement Present"))
...
fig_ads <- ggplot(ads_order_items_df, aes(x = Ads_Present, fill = Ads_Present))+
  geom_bar() +
  labs(title = "Distribution of Ads Presence", x = "Advertisement", 
       y = "Number of Sales") +
...
```{r}
#From the result, advertisement present significantly impacts the order amount. 
#Useful information for the platform to push the sales in a particular month.
```
...
cat_query <- "SELECT a.product_id AS Product_ID, 
a.products_category AS Category, b.order_date AS Order_Date
FROM Products a
...
# Manipulating date, creating month column, counting times category ordered (month)
categories_df <- mutate(categories_df, 
Order_Date = as.POSIXct(categories_df$Order_Date, 
origin = "1970-01-01", tz = "UTC"),
Month = format(Order_Date, "%b")) %>% 
  select(Category, Month) %>% count(Category, Month, name = "Count")

# Ordering months
categories_df$Month <- factor(categories_df$Month, 
levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", 
           "Oct", "Nov", "Dec"))

# Plot showing number of orders placed each month across all categories
fig_cat <- ggplot(categories_df, aes(x = Month, y = Count, group=Category, 
  color=Category)) + geom_point() +geom_line()
...
```{r}
#Plot allows us to observe categories people purchase the most in each month. 
#Beneficial to launch the campaign on each month based on the data.
```

```{r}
#After Analysis, disconnect the database to improve the overall performance 
#By reducing the number of active connections it needs to manage.
...
Additionally, the following geographical heat map shows the delivery locations of shipments. The heat map changes from light blue, green, yellow, and orange to reflect the increasing intensity of sales in a region. It can be seen in areas such as London, Bristol, Edinburgh etc. that sales are high.
...
It would be impactful to persuade suppliers in months when sales are low. From our analysis, in May, June, August and October sales are lower than in other months and suppliers would be incentivised to advertise their products then.

```{r, echo=FALSE}
fig_order
```

To further add value to our analysis, we are able to identify the number of sales across different categories across all months. For example, sales are high for most suppliers in February except for suppliers selling travel essentials. Hence, utilising both the orders by month and categories graphs we can tailor advertisements by month and categories for all suppliers.
...
To sum up, during the initial phase, we established the groundwork for our e-commerce platforms and implemented rigorous data quality checks by creating tables. To streamline the process, we utilised Python to generate data and established automation through GitHub. This allowed us to perform regular verification, updating, and analysis tasks daily. Lastly, we performed various analyses to improve the e-commerce platform’s performance.
...
# 
# # Payment
# num_payments = len(New_Order_items)
# 
# order_item_non_pending = New_Order_items[(New_Order_items['order_item_status'] != 'Pending') & (New_Order_items['order_item_status'] != 'Cancelled')].copy()
# 
# conditions = [
#     order_item_non_pending['order_item_status'] == 'Processing',
#     order_item_non_pending['order_item_status'] == 'Shipped',
...
# 
# 
# order_item_pending = New_Order_items[(New_Order_items['order_item_status'] == 'Pending') | (New_Order_items['order_item_status'] == 'Cancelled')].copy()
# 
# 
# def get_random_payment_status(row):
#     if row['order_item_status'] == 'Processing':
...
# 
# 
# New_Order_items = New_Order_items.drop(columns=['order_item_status'])
# 
# Suppliers2 = pd.concat([Suppliers, New_Suppliers])
# Advertisements2 = pd.concat([Advertisements, New_Advertisements])
# Orders2 = pd.concat([Orders, New_Orders])
# Order_items2 = pd.concat([Order_items, New_Order_items])
# Shipments2 = pd.concat([Shipments, New_Shipments])
# Payments2 = pd.concat([Payments, New_Payments])
# Customers2 = pd.concat([Customers, New_Customers])
# Products2 = pd.concat([Products, New_Products])
# 
# 
# Suppliers2.to_csv('update/Suppliers.csv', index=False)
# Advertisements2.to_csv('update/Advertisements.csv', index=False)
# Orders2.to_csv('update/Orders.csv', index=False)
# Order_items2.to_csv('update/Order_items.csv', index=False)
# Shipments2.to_csv('update/Shipments.csv', index=False)
# Payments2.to_csv('update/Payments.csv', index=False)
# Customers2.to_csv('update/Customers.csv', index=False)
# Products2.to_csv('update/Products.csv', index=False)
